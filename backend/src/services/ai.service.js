...
11. **DSA Optimization:** Analyze the algorithmic complexity and suggest efficient alternatives (e.g., replacing O(n²) algorithms with O(n log n) alternatives when applicable).
...
async function generateContent(code, language) {
    const prompt = `
        You are a senior code reviewer. The user has written code in ${language}.
